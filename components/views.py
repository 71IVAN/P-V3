...
from .models import third, Contact
...
class formSave(View):
    def post(self, request):
        if request.method == 'POST':
            ...
            firstName = request.POST.get('basicpill-firstname-input')
            lastName = request.POST.get('basicpill-lastname-input')
            phone = request.POST.get('basicpill-phoneno-input')
            email = request.POST.get('basicpill-email-input')
            address = request.POST.get('basicpill-address-input')
            thirdPartyType = request.POST.get('basicpill-gender-input')
...
class FormContact(View):
    def get(self, request):
        gretting = {}
        gretting['heading'] = "Form Contact"
        gretting['pageview'] = "Forms"
        return render (request, 'components/forms/components-contact.html', gretting)
    
class SaveContactForm(View):
    def post(self, request):
        if request.method == 'POST':
            #Obtener los datos
            firstname = request.POST.get('firstname')
            lastname = request.POST.get('lastname-contact')
            phone = request.POST.get('phone-contact')
            email= request.POST.get('email-contact')
            address= request.POST.get('address')
            genderAll= request.POST.get('genderAll')

            #Crear una nueva instancia de tu modelo y guarda los datos en la db.
            newObject = Contact(
                firstname=firstname,
                lastname=lastname,
                phone=phone,
            ...
            return render(request, 'components/forms/components-contact.html', {})
